...
-- OPTIMIZED: Reduce yields - only yield every 20 iterations instead of every iteration
local function ReduceStrings(strList)
	local didReduction = true
	local iterationCount = 0  -- OPTIMIZED: Track iterations
	while didReduction do
		...
		-- OPTIMIZED: Only yield every 20 iterations to reduce overhead
...
local function GenerateSearchTerms(names, itemList, isReversed)
	...
	for _, itemString in ipairs(itemList) do
		...
		if itemName then
			itemNameCache[itemString] = isReversed and strrev(itemName) or itemName
		end
	end
